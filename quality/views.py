...
from .models import InspectionItem, InspectionRecord, DefectiveProduct, FinalInspectionReport, AOITestReport
...
quality_handler.setFormatter(logging.Formatter('%(levelname)s %(asctime)s %(module)s %(message)s'))
...
def inspection_items(request):
    ...
    return render(request, 'quality/inspection_items.html', {
        'inspection_items': inspection_items,
    })
...
def inspection_records(request):
    ...
    return render(request, 'quality/inspection_records.html', {
        'inspection_records': inspection_records,
    })
...
def defective_products(request):
    ...
    return render(request, 'quality/defective_products.html', {
        'defective_products': defective_products,
    })
...
def final_inspection(request):
    ...
    return render(request, 'quality/final_inspection.html', {
        'final_inspections': final_inspections,
    })
...
def aoi_test_report(request):
    ...
    return render(request, 'quality/aoi_test_report.html', {
        'aoi_test_reports': aoi_test_reports,
    })
...
def get_final_inspections(request):
    log_user_operation(request.user.username, 'quality', "通過 API 獲取製成檢驗/入庫檢驗表列表")
    final_inspections = FinalInspectionReport.objects.all()
...
def get_aoi_test_reports(request):
    log_user_operation(request.user.username, 'quality', "通過 API 獲取 AOI 測試報告列表")
    aoi_test_reports = AOITestReport.objects.all()
