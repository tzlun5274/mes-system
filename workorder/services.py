...
class WorkOrderProductionService:
    """
    工單生產管理服務
    提供工單生產相關的業務邏輯
    """
    ...
    @staticmethod
    def get_production_summary(workorder):
        """
        取得工單的生產摘要資訊
        """
        try:
            production_record = WorkOrderProduction.objects.get(workorder=workorder)
            details = production_record.production_details.all()
            
            total_work_quantity = sum(detail.work_quantity for detail in details)
            total_defect_quantity = sum(detail.defect_quantity for detail in details)
            total_reports = details.count()
            
            return {
                'production_status': production_record.status,
                'total_work_quantity': total_work_quantity,
                'total_defect_quantity': total_defect_quantity,
                'total_reports': total_reports,
                'production_start_date': production_record.production_start_date,
                'current_process': production_record.current_process,
            }
        except WorkOrderProduction.DoesNotExist:
            return None 