    ...
    def handle(self, *args, **options):
        ...
        if start_date and end_date:
            queryset = queryset.filter(
                report_time__date__range=[start_date, end_date]
            )
        ...
        if total_count == 0:
            self.stdout.write(
                self.style.SUCCESS('✅ 沒有找到符合條件的 SMT 報工記錄')
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'📊 找到 {total_count} 筆 SMT 報工記錄 ({date_filter})'
            )
        )
        ...
        for report in queryset:
            ...
            if (equipment_stats[equipment_name]['latest_report'] is None or 
                report.report_time > equipment_stats[equipment_name]['latest_report']):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'📊 清除後剩餘 SMT 報工記錄：{remaining_count} 筆'
            )
        )
