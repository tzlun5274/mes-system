...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not smt_equipment.exists():
            self.stdout.write(self.style.WARNING("⚠️  沒有找到 SMT 設備"))
            return
        ...
        # 顯示當前狀態
        self.stdout.write(self.style.SUCCESS("📊 當前 SMT 系統狀態："))
        ...
        if options["dry_run"]:
            ...
            if options["reset_equipment"]:
                self.stdout.write(
                    self.style.WARNING(f"  • 將重置所有 SMT 設備狀態為閒置\n")
                )

            self.stdout.write(self.style.WARNING("使用 --confirm 參數實際執行重置操作"))
            return
        ...
        try:
            ...
            if total_reports > 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✅ 成功清除 {deleted_count} 筆 SMT 報工記錄")
                )
            else:
                self.stdout.write(self.style.SUCCESS("✅ 沒有 SMT 報工記錄需要清除"))
            ...
            if options["reset_equipment"]:
                ...
                if updated_count > 0:
                    ...
                    self.stdout.write(self.style.SUCCESS("✅ 所有設備狀態已經是閒置"))
