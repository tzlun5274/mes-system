...
from workorder.models import WorkOrder, ManufacturingOrder
...
class Command(BaseCommand):
    help = "自動同步各公司製造命令到 ManufacturingOrder 表"
        ...
        for company in companies:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    # 更新或建立 ManufacturingOrder 記錄
                    manufacturing_order, created = ManufacturingOrder.objects.update_or_create(
                ...
                workorder_logger.info(
                    f"公司 {company.company_name} 手動同步製造命令完成，新增 {company_synced} 筆記錄。"
                )
                ...
                workorder_logger.error(
                    f"公司 {company.company_name} 手動同步製造命令失敗：{e}"
                )

        self.stdout.write(
            self.style.SUCCESS(
                f"所有公司同步完成，共新增 {total_synced} 筆公司製造命令記錄"
            )
        )
        workorder_logger.info(
            f"所有公司手動同步製造命令完成，共新增 {total_synced} 筆公司製造命令記錄。"
        )
