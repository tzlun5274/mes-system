...
def company_list_api(request):
    """公司列表 API - 使用 WorkOrderDispatch 模型以保持與填報功能一致"""
    try:
        from workorder.workorder_dispatch.models import WorkOrderDispatch
        
        # 取得所有公司代號
        companies = WorkOrderDispatch.objects.values('company_code').distinct().order_by('company_code')
...
@login_required
def workorder_list_api(request):
    """工單列表 API - 使用 WorkOrderDispatch 模型以保持與填報功能一致"""
    try:
        from workorder.workorder_dispatch.models import WorkOrderDispatch
        ...
        # 建立查詢條件
        query = WorkOrderDispatch.objects.all()
        
        if product_id:
            query = query.filter(product_code=product_id)
        
        if company_name:
            query = query.filter(company_code=company_name)
        
        # 取得工單資料
        workorders = query.values('order_number', 'company_code', 'planned_quantity').distinct().order_by('order_number')
        ...
        debug_info = {
            ...
            'filtered_count': len(workorder_list),
            'product_filter': product_id,
            'company_filter': company_name,
            'query_sql': str(query.query) if hasattr(query, 'query') else 'N/A'
        }
...
@login_required
def workorder_by_product_api(request):
    """根據產品編號取得工單 API"""
    try:
        ...
        ).values('order_number', 'company_code', 'quantity').distinct()
...
@login_required
def products_by_company_api(request):
    """根據公司名稱取得產品編號 API"""
    try:
        from workorder.models import WorkOrder
        ...
        # 根據公司名稱取得產品編號
        products = WorkOrder.objects.filter(
            company_code=company_name
        ).values('product_code').distinct().order_by('product_code')
...
@login_required
def workorder_detail_api(request):
    """根據工單號碼取得工單詳細資訊 API"""
    try:
        from workorder.models import WorkOrder
