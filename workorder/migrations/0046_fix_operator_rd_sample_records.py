...
def fix_operator_rd_sample_records(apps, schema_editor):
    ...
    inconsistent_records = OperatorSupplementReport.objects.filter(
        report_type="normal", workorder__isnull=True
    )

    # 將這些記錄修正為RD樣品
    if inconsistent_records.exists():
        updated_count = inconsistent_records.update(
            report_type="rd_sample", product_id="RD樣品產品"
        )
        ...
        for record in inconsistent_records:
            print(
                f"  - 記錄ID: {record.id}, 作業員: {record.operator.name if record.operator else 'N/A'}, 日期: {record.work_date}"
            )
    else:
...
def reverse_fix_operator_rd_sample_records(apps, schema_editor):
    ...
    updated_count = rd_records.update(report_type="normal", product_id="")
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            fix_operator_rd_sample_records, reverse_fix_operator_rd_sample_records
        ),
    ]
