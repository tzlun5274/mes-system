"""
公司生產製造命令管理子模組 - 模型定義
負責ERP整合功能，包括公司生產製造命令、ERP生產製造命令主檔等
"""
...
class CompanyOrder(models.Model):
    """
    MES 專用公司生產製造命令主表
    負責生產製造命令的業務管理、狀態管理和工單轉換
    """
    
    # 基本欄位
    company_code = models.CharField(max_length=10, verbose_name="公司代號", help_text="公司代號，例如：01、02、03")
    mkordno = models.CharField(max_length=50, verbose_name="生產製造命令單號", help_text="生產製造命令單號")
    ...
    is_converted = models.BooleanField(default=False, verbose_name="是否已轉換成MES工單", help_text="是否已轉換成MES工單")
    
    # ERP 原始資料欄位（用於同步）
    flag = models.IntegerField(verbose_name="ERP狀態Flag", null=True, blank=True, help_text="ERP狀態Flag")
    mkord_date = models.CharField(max_length=20, verbose_name="生產製造命令日期", blank=True, null=True, help_text="生產製造命令日期")
    make_type = models.CharField(max_length=20, verbose_name="生產製造命令類型", blank=True, null=True, help_text="生產製造命令類型")
    ...
    def __str__(self):
        ...
        return f"[{formatted_company_code}] 生產製造命令 {self.mkordno}"
