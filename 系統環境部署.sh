...
run_command "mkdir -p $LOG_BASE_DIR/gunicorn" "建立 Gunicorn 日誌目錄"
...
run_command "touch $LOG_BASE_DIR/gunicorn/gunicorn.log" "建立 Gunicorn 日誌檔案"
...
run_command "chown mes:www-data $LOG_BASE_DIR/mes.log $LOG_BASE_DIR/workorder.log $LOG_BASE_DIR/django/mes.log $LOG_BASE_DIR/celery/celery.log $LOG_BASE_DIR/gunicorn/gunicorn.log" "設定應用日誌檔案擁有者"
...
run_command "chmod 644 $LOG_BASE_DIR/mes.log $LOG_BASE_DIR/workorder.log $LOG_BASE_DIR/django/mes.log $LOG_BASE_DIR/celery/celery.log $LOG_BASE_DIR/gunicorn/gunicorn.log" "設定應用日誌檔案權限"
...
run_command "find $LOG_BASE_DIR/gunicorn -type f -exec chown mes:www-data {} \;" "設定 Gunicorn 日誌檔案擁有者"
...
check_directory_permissions "$LOG_BASE_DIR/gunicorn" "mes:www-data" "755" "Gunicorn 日誌目錄" || PERMISSION_ERRORS=$((PERMISSION_ERRORS + 1))
...
check_file_permissions "$LOG_BASE_DIR/gunicorn/gunicorn.log" "mes:www-data" "644" "Gunicorn 日誌檔案" || PERMISSION_ERRORS=$((PERMISSION_ERRORS + 1))
...
if [ ! -d "$PROJECT_DIR" ]; then
    ...
    # 動態檢測 gunicorn 和 celery 路徑
    echo "動態檢測 gunicorn 和 celery 路徑..." | tee -a $LOG_FILE
    GUNICORN_PATH=$(which gunicorn)
    CELERY_PATH=$(which celery)
    
    if [ -z "$GUNICORN_PATH" ]; then
        echo -e "${RED}❌ 找不到 gunicorn，請先安裝${NC}" | tee -a $LOG_FILE
        exit 1
    fi
    ...
    echo "  gunicorn: $GUNICORN_PATH" | tee -a $LOG_FILE
    echo "  celery: $CELERY_PATH" | tee -a $LOG_FILE

    # 建立 Gunicorn 服務檔案
    echo "建立 Gunicorn 服務檔案..." | tee -a $LOG_FILE
    cat > /etc/systemd/system/gunicorn-mes_config.service << EOF
[Unit]
Description=Gunicorn instance to serve MES system
...
ExecStart=$GUNICORN_PATH --workers 3 --bind 127.0.0.1:8000 mes_config.wsgi:application
...
if [ -d "$PROJECT_DIR" ] && [ -f "$PROJECT_DIR/manage.py" ]; then
    # 啟動專案服務
    run_command "systemctl enable gunicorn-mes_config" "啟用 Gunicorn 服務"
    ...
    run_command "systemctl status gunicorn-mes_config --no-pager" "檢查 Gunicorn 狀態"
...
echo "Nginx: systemctl status nginx" | tee -a $LOG_FILE
if [ -d "$PROJECT_DIR" ] && [ -f "$PROJECT_DIR/manage.py" ]; then
    echo "Gunicorn: systemctl status gunicorn-mes_config" | tee -a $LOG_FILE
...
if [ -d "$PROJECT_DIR" ] && [ -f "$PROJECT_DIR/manage.py" ]; then
    ...
    echo "Gunicorn 日誌: journalctl -u gunicorn-mes_config" | tee -a $LOG_FILE
fi
echo "" | tee -a $LOG_FILE
echo "部署完成時間: $(date)" | tee -a $LOG_FILE
...
echo "----------------------------------------" | tee -a $LOG_FILE
if [ -d "$PROJECT_DIR" ] && [ -f "$PROJECT_DIR/manage.py" ]; then
    echo "🌐 網站已啟動: http://$HOST_IP" | tee -a $LOG_FILE
    echo "👤 管理員帳號: $SUPERUSER_NAME" | tee -a $LOG_FILE
    echo "📧 管理員郵件: $SUPERUSER_EMAIL" | tee -a $LOG_FILE
else
