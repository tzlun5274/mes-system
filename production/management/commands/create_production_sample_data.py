...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('產線管理範例資料建立完成！')
        )
                ...
                if is_holiday:
                    ...
                    schedule, created = ProductionLineSchedule.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'overtime_end_time': None,
                            'is_holiday': True,
                            'holiday_reason': random.choice(['國定假日', '設備維護', '停電', '特殊假日']),
                            'work_days': '[]'
                        }
                    )
                else:
                    # 正常工作日排程
                    # 根據產線設定決定是否有午休時間
                    has_lunch_break = line.lunch_start_time is not None and line.lunch_end_time is not None
                    ...
                    schedule, created = ProductionLineSchedule.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'lunch_start_time': line.lunch_start_time if has_lunch_break else None,
                            'lunch_end_time': line.lunch_end_time if has_lunch_break else None,
