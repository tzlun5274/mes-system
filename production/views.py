...
from .forms import ProductionLineTypeForm, ProductionLineForm, ProductionLineScheduleForm
import json

# 設定生產管理模組的日誌記錄器
production_logger = logging.getLogger('production')
production_handler = logging.FileHandler('/var/log/mes/production.log')
production_handler.setFormatter(logging.Formatter('%(levelname)s %(asctime)s %(module)s %(message)s'))
...
def index(request):
    ...
    # 最近建立的產線
    recent_lines = ProductionLine.objects.filter(is_active=True).order_by('-created_at')[:5]
...
def schedule_list(request):
    ...
    # 查詢條件
    queryset = ProductionLineSchedule.objects.select_related('production_line', 'production_line__line_type').all()
...
def api_line_types(request):
    ...
    API：取得產線類型列表
    """
    line_types = ProductionLineType.objects.filter(is_active=True).values('id', 'type_code', 'type_name')
