...
class OrderManager:
    ...
    def _sync_domestic_orders(self, cursor, company_name: str) -> List[Dict]:
        ...
        try:
            ...
            return [
                self._format_domestic_order(order, company_name) for order in orders
            ]
...
class OrderQueryManager:
    ...
    def get_filter_options(self) -> Dict:
        ...
        try:
            return {
                "companies": OrderMain.objects.values_list(
                    "company_name", flat=True
                ).distinct(),
                "customers": OrderMain.objects.values_list(
                    "customer_short_name", flat=True
                ).distinct(),
                "order_types": ["國內", "國外"],
            }
        except Exception as e:
    ...
    def export_orders_to_csv(
        self, orders, filename: str = "orders.csv"
    ) -> HttpResponse:
        """
        將訂單資料匯出為 CSV 檔案
...
class OrderScheduleManager:
    ...
    def update_sync_schedule(
        self, sync_interval_minutes: int, user=None, ip_address=None
    ) -> Dict:
        """
        更新客戶訂單同步排程設定
        ...
        try:
            ...
            if sync_interval_minutes < 0:
                return {"status": "error", "message": "同步間隔時間不能為負數"}
            ...
            return {
                "status": "success",
                "message": f"客戶訂單更新排程設定成功，間隔 {sync_interval_minutes} 分鐘",
            }

        except Exception as e:
            self.logger.error(f"更新排程設定失敗: {str(e)}")
            return {"status": "error", "message": f"更新排程設定失敗: {str(e)}"}
        ...
        try:
            ...
            if sync_interval_minutes > 0:
                interval, _ = IntervalSchedule.objects.get_or_create(
                    every=sync_interval_minutes, period=IntervalSchedule.MINUTES
                )
    ...
    def _log_schedule_update(self, user, ip_address: str, sync_interval_minutes: int):
        ...
        SchedulingOperationLog.objects.create(
            user=user.username,
            action=f"更新客戶訂單同步間隔為 {sync_interval_minutes} 分鐘",
            ip_address=ip_address,
            timestamp=timezone.now(),
        )
    ...
    def get_sync_status(self) -> Dict:
        ...
        try:
            ...
            if schedule.last_updated and schedule.sync_interval_minutes > 0:
                next_update = schedule.last_updated + timedelta(
                    minutes=schedule.sync_interval_minutes
                )
...
class OrderAnalytics:
    ...
    def get_order_summary(self) -> Dict:
        ...
        try:
            ...
            total_quantity = (
                OrderMain.objects.aggregate(Sum("quantity"))["quantity__sum"] or 0
            )
            total_remain = (
                OrderMain.objects.aggregate(Sum("qty_remain"))["qty_remain__sum"] or 0
            )
            ...
            return {
                ...
                "total_remain": total_remain,
                "completion_rate": (
                    (total_quantity - total_remain) / total_quantity * 100
                    if total_quantity > 0
                    else 0
                ),
    ...
    def get_delivery_analysis(self) -> Dict:
        ...
        try:
            # 即將到期的訂單（7天內）
            seven_days_later = (timezone.now().date() + timedelta(days=7)).strftime(
                "%Y-%m-%d"
            )
            urgent_orders = OrderMain.objects.filter(
                pre_in_date__lte=seven_days_later, qty_remain__gt=0
            ).count()

            # 已逾期的訂單
            today = timezone.now().date().strftime("%Y-%m-%d")
            overdue_orders = OrderMain.objects.filter(
                pre_in_date__lt=today, qty_remain__gt=0
            ).count()
            ...
            delivery_groups = {
                ...
                "normal": OrderMain.objects.filter(
                    pre_in_date__gt=seven_days_later, qty_remain__gt=0
                ).count(),
            }
