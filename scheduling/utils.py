...
def check_holiday_conflicts(tasks):
    ...
    try:
        holiday_response = requests.get(
            "http://localhost:8000/scheduling/api/holidays/", timeout=10
        )
        ...
        for task in tasks:
            ...
            try:
                ...
                end_datetime = datetime.strptime(end_time, "%Y-%m-%dT%H:%M").replace(
                    tzinfo=TAIWAN_TZ
                )
            ...
            for holiday in holidays:
                ...
                if (
                    start_datetime.date() <= holiday_end.date()
                    and end_datetime.date() >= holiday_start.date()
                ):
                    conflicts.append(
                        {
                            "task": task,
