...
class ProductionSafetySettings(models.Model):
    delivery_to_completion_safety_days = models.IntegerField(
        default=3, verbose_name=_("預定出貨日到完工日安全天數（天）")
    )
    ...
    def __str__(self):
        return (
            f"預定出貨日到完工日安全天數 {self.delivery_to_completion_safety_days} 天"
        )
