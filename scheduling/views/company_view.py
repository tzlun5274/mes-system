...
def company_view(request):
    try:
        ...
        return render(request, "scheduling/company_view.html", {"companies": companies})
    except Exception as e:
        logger.error(
            f"處理 /scheduling/company_view/ 請求時發生錯誤: {str(e)}", exc_info=True
        )
        messages.error(request, _("無法加載公司檢視頁面，請聯繫管理員"))
        return render(request, "scheduling/company_view.html", {"companies": []})
