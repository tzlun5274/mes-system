...
def update_process_interval(request):
    ...
    try:
        process_interval_minutes = request.POST.get('process_interval_minutes', '').strip()
        ...
        if not process_interval_minutes:
            ...
            return JsonResponse({'status': 'error', 'message': '請提供工序間隔時間'}, status=400)
        try:
            process_interval_minutes = int(process_interval_minutes)
            if process_interval_minutes < 0:
                logger.error(f"無效的 process_interval_minutes: {process_interval_minutes}，不能為負數")
                return JsonResponse({'status': 'error', 'message': '工序間隔時間不能為負數'}, status=400)
        except ValueError as ve:
            logger.error(f"無效的 process_interval_minutes: {process_interval_minutes}, 錯誤: {str(ve)}")
            return JsonResponse({'status': 'error', 'message': '請輸入有效的工序間隔時間（非負整數）'}, status=400)

        process_interval_settings, created = ProcessIntervalSettings.objects.get_or_create(id=1)
        ...
        return JsonResponse({'status': 'success', 'message': '工序間隔時間設定成功'}, status=200)
