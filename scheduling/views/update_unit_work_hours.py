...
def update_unit_work_hours(request):
    ...
    try:
        ...
        if not unit_id or not work_start or not work_end:
            messages.error(request, _("請填寫所有必填字段（單位、上班開始時間、下班時間）"))
            return JsonResponse({'status': 'error', 'message': '請填寫所有必填字段'}, status=400)

        unit = Unit.objects.get(id=unit_id)
        # 防呆：禁止修改預設單位名稱
        if unit.name == '預設單位' and request.POST.get('unit_name') and request.POST.get('unit_name') != '預設單位':
            messages.error(request, '預設單位名稱不可修改')
            return JsonResponse({'status': 'error', 'message': '預設單位名稱不可修改'}, status=400)
