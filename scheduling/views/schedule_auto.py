...
from ..algorithms import check_holiday_conflicts, generate_auto_tasks, generate_optimized_auto_tasks
import logging
import requests
import traceback
...
def schedule_auto(request):
    if request.method == 'POST':
        try:
            ...
            clear_old_schedule = request.POST.get('clear_old_schedule', 'false') == 'true'
            ...
            # 獲取訂單
            orders = OrderMain.objects.filter(qty_remain__gt=0, product_id__startswith='PFP-')
            if not orders.exists():
                messages.error(request, _("無可用訂單"))
                return JsonResponse({'status': 'error', 'message': '無可用訂單'}, status=400)
            ...
            safety_days = safety_settings.delivery_to_completion_safety_days if safety_settings else 3
            ...
            for order in orders:
                if order.pre_in_date and order.pre_in_date != 'N/A':
                    try:
                        pre_in_date_dt = datetime.strptime(order.pre_in_date, '%Y-%m-%d').replace(tzinfo=TAIWAN_TZ)
                        if current_time <= pre_in_date_dt - timedelta(days=safety_days):
                            valid_orders.append((order, pre_in_date_dt))
                    except ValueError as e:
                        logger.warning(f"訂單 {order.id} 的 pre_in_date 格式無效: {order.pre_in_date}, 錯誤: {str(e)}")
                        continue
                else:
            ...
            if not valid_orders:
                ...
                return JsonResponse({'status': 'error', 'message': '無符合交期條件的訂單'}, status=400)

            # 按交期排序
            valid_orders.sort(key=lambda x: x[1] if x[1] else datetime.max.replace(tzinfo=TAIWAN_TZ))
            orders = [order for order, _ in valid_orders]

            cookies = {'sessionid': request.COOKIES.get('sessionid', '')}
            
            # 獲取資源數據
            process_response = requests.get('http://localhost:8000/process/api/process_names/', cookies=cookies, timeout=10)
            if process_response.status_code != 200:
                logger.error(f"無法獲取工序數據，狀態碼: {process_response.status_code}")
                return JsonResponse({'status': 'error', 'message': '無法獲取工序數據'}, status=500)
            processes = process_response.json().get('process_names', [])
            
            operator_response = requests.get('http://localhost:8000/process/api/operators/', cookies=cookies, timeout=10)
            if operator_response.status_code != 200:
                logger.error(f"無法獲取作業員數據，狀態碼: {operator_response.status_code}")
                return JsonResponse({'status': 'error', 'message': '無法獲取作業員數據'}, status=500)
            operators = operator_response.json().get('operators', [])
            
            equipment_response = requests.get('http://localhost:8000/equip/api/equipments/', cookies=cookies, timeout=10)
            if equipment_response.status_code != 200:
                logger.error(f"無法獲取設備數據，狀態碼: {equipment_response.status_code}")
                return JsonResponse({'status': 'error', 'message': '無法獲取設備數據'}, status=500)
            equipments = equipment_response.json().get('equipments', [])
            
            smt_equipment_response = requests.get('http://localhost:8000/smt_equipment/api/smt_equipments/', cookies=cookies, timeout=10)
            if smt_equipment_response.status_code != 200:
                logger.error(f"無法獲取 SMT 設備數據，狀態碼: {smt_equipment_response.status_code}")
                return JsonResponse({'status': 'error', 'message': '無法獲取 SMT 設備數據'}, status=500)
            ...
            if use_optimized:
                ...
                routes_by_order = {}
                for order in orders:
                    routes_response = requests.get('http://localhost:8000/process/api/product_routes/', 
                                                 params={'product_id': order.product_id}, cookies=cookies, timeout=10)
                    if routes_response.status_code != 200:
                        failed_orders[order.id] = (f"無法獲取產品工序路線數據: 狀態碼={routes_response.status_code}", 
                                                 "請檢查工序管理中的產品工序路線設定")
                        continue

                    all_routes = routes_response.json().get('product_routes', [])
                    matched_routes = [route for route in all_routes if route['product_id'] == order.product_id]
                    if not matched_routes:
                        failed_orders[order.id] = (f"產品 {order.product_id} 未設定工藝路線", 
                                                 "請在工序管理中設定產品工藝路線")
                        continue
            ...
            for task in all_tasks:
                start_datetime = datetime.strptime(task['start_time'], '%Y-%m-%dT%H:%M').replace(tzinfo=TAIWAN_TZ)
                end_datetime = datetime.strptime(task['end_time'], '%Y-%m-%dT%H:%M').replace(tzinfo=TAIWAN_TZ)
                ...
                Event.objects.create(
                    ...
                    category='general',
                    created_by=request.user.username,
                    employee_id=task['selected_operator']['id'] if task['selected_operator'] else None,
                    equipment_id=task['selected_equipment']['id'] if task['selected_equipment'] else None,
                    smt_equipment_id=task['selected_smt_equipment']['id'] if task['selected_smt_equipment'] else None,
