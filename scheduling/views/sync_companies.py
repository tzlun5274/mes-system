...
def sync_companies(request):
    ...
    try:
        try:
            ...
            erp_companies = CompanyConfig.objects.values('company_name', 'mes_database', 'notes')
            logger.debug(f"從 erp_integration 獲取公司數據: {erp_companies}")
        except LookupError:
            logger.warning("erp_integration 模組未找到")
            return JsonResponse({'status': 'error', 'message': 'ERP整合模組未啟用'}, status=404)
        except Exception as e:
            logger.error(f"讀取 CompanyConfig 失敗: {str(e)}", exc_info=True)
            return JsonResponse({'status': 'error', 'message': '無法讀取公司數據'}, status=500)
        ...
        for company in erp_companies:
            try:
                ...
                logger.error(f"寫入公司數據失敗: {company['company_name']}, 錯誤: {str(e)}", exc_info=True)
                continue
        ...
        logger.error(f"處理 /scheduling/sync_companies/ 請求時發生錯誤: {str(e)}", exc_info=True)
        return JsonResponse({'status': 'error', 'message': f'寫入公司數據失敗: {str(e)}'}, status=500)
