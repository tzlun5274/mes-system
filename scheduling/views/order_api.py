...
def update_sync_schedule_api(request):
    ...
    try:
        ...
        if sync_interval_minutes is None:
            return JsonResponse({
                'status': 'error',
                'message': '缺少 sync_interval_minutes 參數'
            }, status=400)
        ...
        try:
            ...
            return JsonResponse({
                'status': 'error',
                'message': 'sync_interval_minutes 必須為整數'
            }, status=400)
