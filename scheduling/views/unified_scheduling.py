...
def check_resource_conflicts(request):
    ...
    try:
        ...
        if not start_date or not end_date:
            return JsonResponse({"status": "error", "message": "請提供開始和結束日期"})
        ...
        for unit_id in unit_ids:
            try:
                ...
                conflicting_events = Event.objects.filter(
                    unit=unit, start__lt=end_dt, end__gt=start_dt
                    ...
                    unit_conflicts.append(
                        {
                            "unit_name": unit.name,
                            "conflicts": list(
                                conflicting_events.values("title", "start", "end")
                            ),
                        }
                    )
        ...
        return JsonResponse({"status": "error", "message": f"衝突檢查失敗: {str(e)}"})
...
def execute_scheduling(request):
    ...
    try:
        ...
        return JsonResponse({"status": "error", "message": f"排程執行失敗: {str(e)}"})
...
def execute_manual_scheduling(parameters: Dict) -> Dict:
    ...
    try:
        ...
        return {"scheduled_tasks": [], "total_duration": 0, "message": "手動排程完成"}
    except Exception as e:
...
def update_gantt_task(request):
    ...
    try:
        ...
        return JsonResponse({"status": "success", "message": "任務更新成功"})

    except Exception as e:
        logger.error(f"甘特圖任務更新失敗: {str(e)}")
        return JsonResponse({"status": "error", "message": f"任務更新失敗: {str(e)}"})


def check_task_conflicts(
    start_dt: datetime, end_dt: datetime, unit_id: int, exclude_task_id: int = None
) -> List[Dict]:
    """
    檢查任務時間衝突
    """
    ...
    event_query = Event.objects.filter(
        unit_id=unit_id, start__lt=end_dt, end__gt=start_dt
    ).exclude(type="workday")
    ...
    production_query = ProductionEvent.objects.filter(
        unit_id=unit_id, start_time__lt=end_dt, end_time__gt=start_dt
    )
...
def get_scheduling_progress(request):
    ...
    try:
        ...
        if not user_id:
            return JsonResponse({"status": "error", "message": "缺少使用者ID"})
        ...
        return JsonResponse(
            {"status": "success", "progress": progress, "progress_key": progress_key}
        )

    except Exception as e:
        logger.error(f"取得排程進度失敗: {str(e)}")
        return JsonResponse({"status": "error", "message": f"取得進度失敗: {str(e)}"})
...
def get_scheduling_parameters(request):
    ...
    try:
        ...
        return JsonResponse(
            {
                ...
                "process_settings": {
                    "min_interval": (
                        process_settings.min_interval if process_settings else 30
                    ),
                    "max_interval": (
                        process_settings.max_interval if process_settings else 120
                    ),
                },
        ...
        return JsonResponse({"status": "error", "message": f"取得參數失敗: {str(e)}"})
