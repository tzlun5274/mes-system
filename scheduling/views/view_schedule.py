    ...
    try:
        ...
        if order_ids:
            orders = OrderMain.objects.filter(
                id__in=[int(oid) for oid in order_ids]
            ).values("id", "product_id", "pre_in_date")
            order_map = {
                str(order["id"]): order for order in orders if order["product_id"]
            }
        ...
        for event in events:
            ...
            if event.description:
                ...
                if match:
                    ...
                    logger.warning(
                        f"事件 {event.id} 未匹配工序名稱: {repr(description)}"
                    )
            ...
            if order_id in order_map:
                ...
                if order_id not in seen_orders and product_id != "N/A":
                    ...
                    logger.warning(
                        f"事件 {event.id} 的訂單 {order_id} 產品編號為 N/A，跳過"
                    )
        ...
        return render(
            request,
            "scheduling/view_schedule.html",
            {"event_data": event_data, "product_id_choices": product_id_choices},
        )
    except Exception as e:
        logger.error(
            f"處理 /scheduling/view_schedule/ 請求時發生錯誤: {str(e)}", exc_info=True
        )
        messages.error(request, _("無法加載排程，請聯繫管理員"))
        return render(
            request,
            "scheduling/view_schedule.html",
            {"event_data": [], "product_id_choices": []},
        )
...
def delete_order_production_events(request):
    ...
    try:
        deleted_count, _ = Event.objects.filter(
            type="production", order_id=order_id
        ).delete()
        SchedulingOperationLog.objects.create(
            user=request.user.username,
