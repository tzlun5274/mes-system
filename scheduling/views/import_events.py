    ...
    if request.method == 'POST':
        try:
            ...
            if not file:
                ...
                return JsonResponse({'status': 'error', 'message': '請選擇一個 CSV 檔案'}, status=400)

            if not file.name.endswith('.csv'):
                messages.error(request, gettext_lazy("檔案格式必須為 CSV"))
                return JsonResponse({'status': 'error', 'message': '檔案格式必須為 CSV'}, status=400)

            try:
                file_data = file.read().decode('utf-8-sig')  # 處理 UTF-8 BOM
            except UnicodeDecodeError:
                messages.error(request, gettext_lazy("無法解碼 CSV 檔案，請確保檔案使用 UTF-8 編碼"))
                return JsonResponse({'status': 'error', 'message': '無法解碼 CSV 檔案'}, status=400)

            csv_data = csv.DictReader(io.StringIO(file_data))
            expected_headers = ['subject', 'start date', 'end date', 'all day event']
            actual_headers = [header.strip().lower() for header in csv_data.fieldnames if header]
            missing_headers = [header for header in expected_headers if header not in actual_headers]
            if missing_headers:
                messages.error(request, gettext_lazy(f"CSV 缺少必要欄位: {', '.join(missing_headers)}"))
                return JsonResponse({'status': 'error', 'message': f"CSV 缺少必要欄位: {', '.join(missing_headers)}"}, status=400)

            header_mapping = {header.strip().lower(): header for header in csv_data.fieldnames if header}
            ...
            for row in csv_data:
                try:
                    ...
                    try:
                        start_date = datetime.strptime(start_date_str, '%Y/%m/%d').replace(tzinfo=TAIWAN_TZ)
                        end_date = datetime.strptime(end_date_str, '%Y/%m/%d').replace(tzinfo=TAIWAN_TZ)
                    ...
                    errors.append(f"事件 '{row.get(subject_key, '未知')}': 數據無效 ({str(e)})")
                    continue
            ...
            if created_count > 0:
                messages.success(request, gettext_lazy(f"成功匯入 {created_count} 筆事件"))
            if errors:
                messages.warning(request, gettext_lazy(f"以下事件匯入失敗: {'; '.join(errors)}"))
        ...
        return render(request, 'scheduling/import_events.html', {'show_duplicate_events': False})
