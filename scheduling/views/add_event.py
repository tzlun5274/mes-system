...
def add_event(request):
    ...
    try:
        ...
        if not title or not start or not end or not event_type:
            messages.error(request, gettext_lazy("請填寫所有必填字段（標題、開始時間、結束時間、類型）"))
            return JsonResponse({'status': 'error', 'message': '請填寫所有必填字段'}, status=400)

        try:
            start_dt = datetime.strptime(start, '%Y-%m-%dT%H:%M').replace(tzinfo=timezone.get_current_timezone())
            end_dt = datetime.strptime(end, '%Y-%m-%dT%H:%M').replace(tzinfo=timezone.get_current_timezone())
        except ValueError as e:
            messages.error(request, gettext_lazy("時間格式無效，請使用 YYYY-MM-DDTHH:MM 格式"))
