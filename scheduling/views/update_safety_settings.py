...
def update_safety_settings(request):
    ...
    try:
        ...
        if not safety_days:
            ...
            return JsonResponse({'status': 'error', 'message': '請提供安全天數'}, status=400)
        try:
            safety_days = int(safety_days)
            if safety_days < 0:
                logger.error(f"無效的 safety_days: {safety_days}，不能為負數")
                return JsonResponse({'status': 'error', 'message': '安全天數不能為負數'}, status=400)
        except ValueError as ve:
            logger.error(f"無效的 safety_days: {safety_days}, 錯誤: {str(ve)}")
            return JsonResponse({'status': 'error', 'message': '請輸入有效的安全天數（非負整數）'}, status=400)
        ...
        return JsonResponse({'status': 'success', 'message': '安全天數設定成功'}, status=200)
