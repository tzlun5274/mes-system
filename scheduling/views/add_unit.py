...
def add_unit(request):
    ...
    try:
        ...
        if not unit_name or not work_start or not work_end:
            messages.error(request, _("請填寫所有必填字段（單位名稱、上班開始時間、下班時間）"))
        ...
        if Unit.objects.filter(name=unit_name).exists():
            ...
            return JsonResponse({'status': 'error', 'message': '單位名稱已存在'}, status=400)

        try:
            work_start_time = datetime.strptime(work_start, '%H:%M').time()
            work_end_time = datetime.strptime(work_end, '%H:%M').time()
            lunch_start_time = datetime.strptime(lunch_start, '%H:%M').time() if has_lunch_break and lunch_start else None
            lunch_end_time = datetime.strptime(lunch_end, '%H:%M').time() if has_lunch_break and lunch_end else None
            overtime_start_time = datetime.strptime(overtime_start, '%H:%M').time() if overtime_start else None
            overtime_end_time = datetime.strptime(overtime_end, '%H:%M').time() if overtime_end else None
