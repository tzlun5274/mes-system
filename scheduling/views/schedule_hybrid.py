...
def schedule_hybrid(request):
    ...
    if request.method == 'POST':
        with open('/tmp/schedule_hybrid_view.log', 'a') as f:
            f.write(f"進入 schedule_hybrid view, user: {request.user.username}, time: {datetime.now()}\n")
        try:
            # 獲取排程參數
            ...
            for task in tasks:
                try:
                    event = Event.objects.create(
                        ...
                        unit=None,
                        start=datetime.strptime(task['start_time'], '%Y-%m-%dT%H:%M').replace(tzinfo=TAIWAN_TZ),
                        end=datetime.strptime(task['end_time'], '%Y-%m-%dT%H:%M').replace(tzinfo=TAIWAN_TZ),
                        type='production',
            ...
            # 若有部分訂單失敗，回傳詳細清單
            if suggestion and isinstance(suggestion, dict) and suggestion.get('failed_orders'):
    ...
    try:
        ...
        for group_name, group in groups.items():
            group_stats[group_name] = {
                ...
                'avg_qty': sum(order.qty_remain for order in group.orders) / len(group.orders) if group.orders else 0
            }
        ...
        return render(request, 'scheduling/schedule_hybrid.html', {
...
def get_scheduling_data(request):
    ...
    try:
        ...
        if smt_equipment_response.status_code == 200:
            ...
            logger.error(f"無法獲取SMT設備數據，狀態碼: {smt_equipment_response.status_code}")
    ...
    try:
        ...
        return JsonResponse({
            ...
            'preview_data': {
                ...
                'statistics': stats,
                'sample_tasks': tasks[:5] if tasks else []  # 只返回前5個任務作為預覽
            }
        })
...
def schedule_warning_board(request):
    ...
    return render(request, 'scheduling/schedule_warning_board.html', {'warnings': warnings}) 