...
def schedule_semi_auto(request):
    def handle_error_response(request, orders, processes, operators, equipments, smt_equipments, selected_order_id, mode, error_message, log_level='error'):
        if log_level == 'error':
            logger.error(error_message)
        elif log_level == 'warning':
            logger.warning(error_message)
        messages.error(request, error_message)
        return render(request, 'scheduling/schedule_semi_auto.html', {
            'orders': orders,
            'processes': processes,
            'operators': operators,
        ...
        try:
            ...
            for t in tasks:
                if 'order_id' not in t:
                    t['order_id'] = request.POST.get('order_id') if isinstance(request.POST.get('order_id'), str) else (request.POST.getlist('order_id')[0] if request.POST.getlist('order_id') else None)
                # 解析 overtime 為布林值
                t['overtime'] = str(t.get('overtime', '')).lower() in ['on', 'true', '1']
                # 保存分批資訊
                t['splits'] = []
            ...
            for oid, tlist in tasks_by_order.items():
                ...
                if not order:
                    ...
                    return JsonResponse({'status': 'error', 'message': f'訂單 {oid} 不存在'}, status=404)
                order_qty = order.qty_remain
                for idx, task in enumerate(tlist):
                    # 檢查分批數量總和是否等於訂單數量
                    total_split_qty = sum(int(split.get('split_qty', 0)) for split in task.get('splits', []))
                    if total_split_qty != order_qty:
                        pname = task.get('process_name', f'步驟{idx+1}')
                        messages.error(request, _(f"訂單 {order.product_id} 第{idx+1}道工序（{pname}）分批數量總和必須等於訂單未出數量"))
                        return JsonResponse({'status': 'error', 'message': f'訂單 {order.product_id} 第{idx+1}道工序（{pname}）分批數量總和必須等於訂單未出數量'}, status=400)
            ...
            for t in tasks:
                ...
                if not process_id:
                    ...
                    return JsonResponse({'status': 'error', 'message': f'訂單 {oid} 任務缺少工序 ID'}, status=400)
                ...
                process = next((p for p in processes if p['id'] == int(process_id)), None)
                if not process:
                    messages.error(request, _(f"訂單 {oid} 無效的工序 ID"))
                    return JsonResponse({'status': 'error', 'message': f'訂單 {oid} 無效的工序 ID'}, status=400)
                ...
                for split in splits:
                    try:
                        ...
                        if not all([start_time, end_time, split_qty]):
                            ...
                            return JsonResponse({'status': 'error', 'message': f'訂單 {oid} 分批缺少必要欄位'}, status=400)
    ...
    try:
        ...
        if process_response.status_code != 200:
            logger.error(f"無法獲取工序數據，狀態碼: {process_response.status_code}")
            return handle_error_response(
                request, valid_orders, [], [], [], [], None, 'semi_auto',
        ...
        if selected_order_id:
            logger.info(f"[重新計算] 觸發半自動排程，order_id={selected_order_id}，user={request.user.username}")
            ...
            if order:
                try:
                    ...
                    logger.debug(f"產品路線 API 響應狀態碼: {routes_response.status_code}")
                    if routes_response.status_code != 200:
                        logger.error(f"無法獲取產品路線，狀態碼: {routes_response.status_code}")
                        return handle_error_response(
                            request, valid_orders, processes, operators, equipments, smt_equipments, selected_order_id, 'semi_auto',
                    ...
                    global_schedule_time = current_time.replace(hour=8, minute=30, second=0, microsecond=0)
                    ...
                    logger.debug(f"生成任務數量: {len(tasks)}, 原因: {reason}, 建議: {suggestion}")
