...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.WARNING("開始清理 ScheduleWarning，只保留最近 1 萬筆...")
        )
        ...
        while count > batch:
            ids = list(
                ScheduleWarning.objects.order_by("created_at").values_list(
                    "id", flat=True
                )[:batch]
            )
        ...
        self.stdout.write(
            self.style.SUCCESS("清理完成！目前 ScheduleWarning 只剩下最近 1 萬筆。")
        )
