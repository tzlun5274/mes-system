...
def index(request):
    ...
    try:
        ...
        # 最近的材料需求
        recent_requirements = MaterialRequirement.objects.select_related(
            "product", "material"
        ).order_by("-id")[:5]
        ...
        for company in CompanyConfig.objects.all():
            ...
            company_materials = Material.objects.filter(
                company_code=company.company_code
            )
            material_count = company_materials.count()
            # 低庫存（假設有 safety_stock 欄位，若無請告知）
            low_stock_count = MaterialInventoryManagement.objects.filter(
                material__company_code=company.company_code,
    ...
    if request.method == "POST":
        ...
        if product_id and quantity:
            try:
                ...
                return render(
                    request,
                    "material/material_requirement_calculation.html",
                    {
                        "product": product,
                        "quantity": quantity,
...
def shortage_alerts(request):
    ...
    """
    alerts = (
        MaterialShortageAlert.objects.select_related("material")
        .all()
        .order_by("-created_at")
    )
...
def resolve_shortage_alert(request, alert_id):
    ...
    return render(
        request,
        "material/resolve_shortage_alert.html",
        {
            "alert": alert,
        },
    )
...
def supply_plan_list(request):
    ...
    try:
        supply_plans = (
            MaterialSupplyPlan.objects.select_related("material")
            .all()
            .order_by("-created_at")
        )
        ...
        return render(
            request,
            "material/supply_plan_list.html",
            {
...
def kanban_list(request):
    ...
    try:
        kanbans = (
            MaterialKanban.objects.select_related("material")
            .all()
...
def update_kanban_status(request, kanban_id):
    ...
    if request.method == "POST":
        ...
        if new_status in ["normal", "warning", "critical"]:
            ...
            messages.success(
                request, f"看板 '{kanban.material.name}' 狀態已更新為 {new_status}"
            )
            return redirect("material:kanban_list")

    return render(
        request,
        "material/update_kanban_status.html",
        {
            "kanban": kanban,
        },
    )
...
def api_check_shortage(request):
    ...
    try:
        ...
        alerts = MaterialShortageAlert.objects.filter(
            material=material, is_resolved=False
        )
...
def inventory_management(request):
    ...
    """
    inventories = (
        MaterialInventoryManagement.objects.select_related("material")
        .all()
        .order_by("material__name")
    )
    ...
    return render(
        request,
        "material/inventory_management.html",
        {
...
def inventory_detail(request, inventory_id):
    ...
    return render(
        request,
        "material/inventory_detail.html",
        {
            "inventory": inventory,
...
def add_inventory_transaction(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                f"交易記錄已新增：{material.name} {transaction_type} {quantity}",
            )
    ...
    # GET 請求：顯示表單
    materials = Material.objects.all().order_by("name")
    warehouses = MaterialInventoryManagement.objects.values_list(
        "warehouse", flat=True
    ).distinct()

    return render(
        request,
        "material/add_inventory_transaction.html",
        {
            "materials": materials,
...
def requirement_estimation(request):
    ...
    """
    estimations = (
        MaterialRequirementEstimation.objects.select_related("material")
        .all()
        .order_by("-estimation_date")
    )
    ...
    return render(
        request,
        "material/requirement_estimation.html",
        {
...
def create_requirement_estimation(request):
    ...
    return render(
        request,
        "material/create_requirement_estimation.html",
        {
            "materials": materials,
...
def estimation_detail(request, estimation_id):
    ...
    return render(
        request,
        "material/estimation_detail.html",
        {
...
def api_inventory_status(request):
    ...
    try:
        ...
        low_stock = MaterialInventoryManagement.objects.filter(
            stock_status="low"
        ).count()
        out_of_stock = MaterialInventoryManagement.objects.filter(
            stock_status="out"
        ).count()
        excess_stock = MaterialInventoryManagement.objects.filter(
            stock_status="excess"
        ).count()
...
def api_requirement_estimation_summary(request):
    ...
    try:
        ...
        confirmed = MaterialRequirementEstimation.objects.filter(
            status="confirmed"
        ).count()
        completed = MaterialRequirementEstimation.objects.filter(
            status="completed"
        ).count()
