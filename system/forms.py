...
class CustomUserCreationForm(UserCreationForm):
    email = forms.EmailField(label="電子郵件", required=False, help_text="可選填，提供您的電子郵件地址。")
...
class EmailConfigForm(forms.Form):
    ...
    email_port = forms.IntegerField(label="郵件端口", min_value=1, max_value=65535, required=True)
    email_use_tls = forms.BooleanField(label="使用 TLS", required=False, initial=True)
    email_host_user = forms.CharField(label="使用者名稱", max_length=100, required=False)
    email_host_password = forms.CharField(label="密碼", max_length=100, required=False, widget=forms.PasswordInput)
    default_from_email = forms.EmailField(label="預設發件人地址", max_length=254, required=True)
