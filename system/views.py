...
from .forms import EmailConfigForm, CustomUserCreationForm, BackupScheduleForm, OperationLogConfigForm
from .models import EmailConfig, BackupSchedule, OperationLogConfig
...
system_handler.setFormatter(logging.Formatter('%(levelname)s %(asctime)s %(module)s %(message)s'))
...
def get_all_permissions():
    all_modules = [
        'equip', 'material', 'scheduling',
        'process', 'quality', 'work_order', 'reporting', 'kanban', 'erp_integration', 'ai'
    ]
...
class CustomPermissionChoiceField(forms.ModelMultipleChoiceField):
    def label_from_instance(self, obj):
        ...
        translations_lower = {k.lower().strip(): v for k, v in PERMISSION_NAME_TRANSLATIONS.items()}
        translated_name = translations_lower.get(name_lower, obj.name)
        if translated_name == obj.name:
            logger.warning(f"未找到權限 '{obj.name}' (codename: {obj.codename}) 的中文翻譯，映射表鍵: {list(translations_lower.keys())}")
            ...
            if translated_name == obj.name:
                ...
                logger.debug(f"通過 codename 找到翻譯: '{obj.codename}' 翻譯為: {translated_name}")
        else:
...
class GroupForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if self.instance and self.instance.name in DEFAULT_GROUPS and name != self.instance.name:
...
def user_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.info(f"用戶 {form.cleaned_data['username']} 由 {request.user.username} 新增")
...
def user_edit(request, user_id):
    ...
    return render(request, 'system/user_form.html', {'form': form, 'title': '編輯用戶', 'user_id': user_id})
...
def user_change_password(request, user_id):
    ...
    return render(request, 'system/user_change_password.html', {'form': form, 'title': '更改用戶密碼', 'user_id': user_id})
...
def group_list(request):
    ...
    display_groups = []
    for group in groups:
        module_name = GROUP_TO_MODULE_MAP.get(group.name, group.name.replace('使用者', '').lower())
...
def group_add(request):
    if request.method == 'POST':
        ...
        form = GroupForm()
    return render(request, 'system/group_form.html', {'form': form, 'title': '新增群組'})
...
def group_edit(request, group_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            permissions = form.cleaned_data['permissions']
            group.permissions.set(permissions)
            module_name = GROUP_TO_MODULE_MAP.get(group_name, group_name.replace('使用者', '').lower())
            display_name = get_module_display_name(module_name)
            messages.success(request, f'群組 {display_name} 編輯成功！')
            logger.info(f"群組 {group_name} 由 {request.user.username} 編輯")
            return redirect('system:group_list')
    else:
        form = GroupForm(instance=group, initial={'permissions': group.permissions.all()})
    return render(request, 'system/group_form.html', {'form': form, 'title': '編輯群組'})
...
def email_config(request):
    ...
    if request.method == 'POST':
        ...
        if 'send_test_email' in request.POST:
            try:
                ...
                logger.info(f"準備發送測試郵件給 Admin 使用者: {admin_user.username}, 目標郵箱: {admin_user.email}")
                if not admin_user.email:
                    logger.error("Admin 使用者未設置電子郵件地址")
                    messages.error(request, 'Admin 使用者未設置電子郵件地址，請先設置！')
                    return redirect('system:email_config')
                subject = 'MES 系統 - 測試郵件'
                message = '這是一封來自 MES 系統的測試郵件。\n\n如果您收到此郵件，表示郵件主機設置正確。'
                from_email = email_config.default_from_email
                recipient_list = [admin_user.email]
                logger.info(f"SMTP 配置: host={email_config.email_host}, port={email_config.email_port}, use_tls={email_config.email_use_tls}, user={email_config.email_host_user}, from_email={from_email}")
                ...
                messages.error(request, f'SMTP 認證失敗，請檢查郵件主機 hesap戶或應用程式密碼：{str(e)}')
        ...
        if form.is_valid():
            ...
            email_config.email_port = form.cleaned_data['email_port'] if form.cleaned_data['email_port'] else 25
    ...
    return render(request, 'system/email_config.html', {'form': form, 'title': '郵件主機設定'})
...
def backup_database(request):
    ...
    return render(request, 'system/backup.html', {'backup_files': backup_files, 'title': '資料庫備份'})
...
def backup_schedule(request):
    ...
    try:
        ...
        task_status = "未啟用"
        task_time = None
    return render(request, 'system/backup_schedule.html', {
        'form': form,
        'title': '自動備份排程',
        ...
        try:
            ...
            if csv_file.name.endswith('.csv'):
                ...
                for row in csv_reader:
                    ...
                    email = row.get('email', '')
                    password = str(row.get('password', default_password)) if row.get('password') is not None else default_password
                    if not username:
                        continue
                ...
                for row in ws.iter_rows(min_row=2, values_only=True):
                    ...
                    password = str(row_data.get('password', default_password)) if row_data.get('password') is not None else default_password
                    if not username:
                        continue
                    user, created = User.objects.get_or_create(username=username)
                    user.email = email
                    ...
                    if created:
                        ...
                        updated_count += 1
            logger.info(f"用戶匯入完成：新增 {created_count} 個，更新 {updated_count} 個")
            messages.success(request, f'用戶匯入完成：新增 {created_count} 個，更新 {updated_count} 個')
        except Exception as e:
...
def operation_log_manage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'操作紀錄保留天數已更新為 {config.retain_days} 天！')
    ...
    for module_name, model_path in MODULE_LOG_MODELS.items():
        try:
            ...
            logger.error(f"無法導入模組 {module_name} 的日誌模型 {model_path}: {str(e)}")
    ...
    return render(request, 'system/operation_log_manage.html', {
        'form': form,
        'logs': logs,
...
def clean_operation_logs(request):
    ...
    for module, model_path in MODULE_LOG_MODELS.items():
        try:
            ...
            log_model = getattr(module, class_name)
            deleted_count, _ = log_model.objects.filter(timestamp__lt=cutoff_date).delete()
            total_deleted += deleted_count
            logger.info(f"清理模組 {module} 的過期操作紀錄，刪除 {deleted_count} 條記錄")
        except Exception as e:
...
def change_password(request):
    ...
    return render(request, 'system/change_password.html', {'form': form, 'title': '變更密碼'})
...
def environment_management(request):
            ...
            if target_env in environments:
                try:
                    ...
                    if os.path.exists(env_file):
                        ...
                        messages.success(request, f'已切換到 {environments[target_env]["name"]}')
                        
                        # 記錄操作日誌
                        logger.info(f'系統管理員 {request.user.username} 將環境從 {current_env} 切換到 {target_env}')
                        
                        # 提示重啟服務
                        messages.warning(request, '請重啟 Django 服務以套用新的環境設定')
                    else:
    ...
    # 獲取日誌檔案信息
    log_dir = getattr(settings, 'LOG_DIR', os.path.join(os.path.dirname(settings.BASE_DIR), 'logs'))
    log_files = []
    ...
    if request.method == 'POST':
        ...
        for log_file in log_files:
            ...
            if mtime < cutoff_date:
                if dry_run:
                    messages.info(request, f'將刪除: {os.path.basename(log_file)} ({mtime.strftime("%Y-%m-%d")})')
                else:
                    try:
                        ...
                        deleted_count += 1
                        deleted_size += size
                        messages.success(request, f'已刪除: {os.path.basename(log_file)}')
                    except Exception as e:
                        messages.error(request, f'刪除失敗: {os.path.basename(log_file)} - {str(e)}')
        
        if dry_run:
            messages.info(request, f'模擬完成，將刪除 {len([f for f in log_files if datetime.datetime.fromtimestamp(os.stat(f).st_mtime) < cutoff_date])} 個檔案')
        else:
            messages.success(request, f'清理完成，刪除了 {deleted_count} 個檔案，釋放 {deleted_size / (1024*1024):.1f} MB 空間')
            logger.info(f'系統管理員 {request.user.username} 清理了 {deleted_count} 個日誌檔案')
