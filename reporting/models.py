...
class ProductionDailyReport(models.Model):
    ...
    operator_name = models.CharField(max_length=100, verbose_name="作業員姓名", default="未指定作業員")
    equipment_name = models.CharField(max_length=100, verbose_name="設備名稱", default="未分配設備")
    line = models.CharField(max_length=50, verbose_name="生產線", choices=[
        ('SMT1', 'SMT 線 1'),
        ('SMT2', 'SMT 線 2'),
        ('SMT3', 'SMT 線 3'),
        ('TEST', '測試設備'),
    ...
    process_name = models.CharField(max_length=100, verbose_name="工序名稱", default="", blank=True)
    ...
    def calculate_from_supplement_logs(cls, date, operator_name=None):
        ...
        # 如果指定作業員，則篩選該作業員的記錄
        if operator_name:
            supplement_logs = supplement_logs.filter(
        ...
        for log in supplement_logs:
            ...
            operator_stats[operator_name]['completed_quantity'] += log.completed_quantity
            ...
            if log.end_time and log.supplement_time:
                ...
                operator_stats[operator_name]['work_hours'] += duration.total_seconds() / 3600
        ...
        for operator_name, stats in operator_stats.items():
            ...
            report, created = cls.objects.update_or_create(
                ...
                defaults={
                    ...
                    'completion_rate': 100.0 if stats['completed_quantity'] > 0 else 0.0,
...
class OperatorPerformance(models.Model):
    ...
    # 工單（新增）
    work_order = models.CharField(max_length=100, verbose_name="工單", default="", blank=True)
    # 產品名稱（新增）
    product_name = models.CharField(max_length=100, verbose_name="產品名稱", default="", blank=True)
    # 開始時間（新增）
