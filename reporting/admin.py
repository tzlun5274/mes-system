...
from .models import ProductionDailyReport, OperatorPerformance, ReportSyncSettings, ReportEmailSchedule, ReportEmailLog

@admin.register(ProductionDailyReport)
class ProductionDailyReportAdmin(admin.ModelAdmin):
    list_display = ['date', 'operator_name', 'equipment_name', 'line', 'completed_quantity', 'work_hours', 'efficiency_rate']
...
class OperatorPerformanceAdmin(admin.ModelAdmin):
    list_display = ['operator_name', 'equipment_name', 'production_quantity', 'equipment_usage_rate', 'date']
...
class ReportEmailScheduleAdmin(admin.ModelAdmin):
    list_display = ['report_type', 'schedule_type', 'send_time', 'is_active', 'updated_at']
    ...
    fieldsets = (
        ...
        ('收件人設定', {
            'fields': ('recipients', 'cc_recipients', 'subject_template')
        }),
        ('時間記錄', {
...
class ReportEmailLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本資訊', {
            'fields': ('schedule', 'report_type', 'recipients', 'subject')
        }),
        ('發送狀態', {
