...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                if options['date_range']:
                    try:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f'📅 將清除 {start_date} 到 {end_date} 的資料')
                        )
                    except ValueError:
                        raise CommandError('日期格式錯誤，請使用 YYYY-MM-DD,YYYY-MM-DD 格式')
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ 清除資料時發生錯誤：{str(e)}')
            )
    ...
    def _clear_reports_only(self, date_filter):
        ...
        # 清除生產日報表
        if date_filter:
            deleted_count = ProductionDailyReport.objects.filter(**date_filter).delete()[0]
        else:
        ...
        # 清除作業員績效報表
        if date_filter:
            deleted_count = OperatorPerformance.objects.filter(**date_filter).delete()[0]
        else:
