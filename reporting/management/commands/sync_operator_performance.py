...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for log in WorkOrderProcessSupplementLog.objects.filter(
            is_approved=True, action_type="complete"
        ):
            ...
            if equipment_id:
                try:
                    ...
                    equipment_name = (
                        equipment_obj.name if equipment_obj else str(equipment_id)
                    )
                except Exception:
            ...
            work_order = (
                process.workorder.order_number
                if hasattr(process, "workorder") and process.workorder
                else ""
            )
            product_name = (
                process.workorder.product_code
                if hasattr(process, "workorder") and process.workorder
                else ""
            )
            process_name = (
                process.process_name if hasattr(process, "process_name") else ""
            )
        ...
        for (
            ...
            if is_final_day_only:
                ...
                total_hours = sum(
                    (
                        ((l.end_time - l.supplement_time).total_seconds() / 3600)
                        if l.end_time and l.supplement_time
                        else 0
                    )
                    for l in logs
                )
                ...
                if total_qty > 0 and any(
                    (l.completed_quantity or 0) == 0 for l in logs
                ):
                    for log in logs:
                        date = log.supplement_time.date()
                        hours = (
                            (
                                (log.end_time - log.supplement_time).total_seconds()
                                / 3600
                            )
                        ...
                        qty = (
                            int(round(total_qty * hours / total_hours))
                            if total_hours > 0
                            else 0
                        )
                        OperatorPerformance.objects.update_or_create(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"已同步 {count} 筆作業員生產報表資料（含自訂關鍵字分攤邏輯）"
            )
        )
