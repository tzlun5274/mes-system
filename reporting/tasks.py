...
from .models import (
    ProductionDailyReport, OperatorPerformance,
    ReportEmailSchedule, ReportEmailLog
)
...
def send_daily_reports_email():
    ...
    for schedule in schedules:
        try:
            ...
            # 生成郵件內容
            subject, message, html_message = generate_email_content(schedule, report_data)
            ...
            log_email_send(schedule, f"MES 系統 - {schedule.get_report_type_display()}", False, str(e))
...
def generate_excel_report(report_type, data):
    ...
    if report_type == 'operator':
        ...
        for row_num, report in enumerate(data, 2):
            ...
            worksheet.cell(row=row_num, column=5, value=report.date.strftime('%Y-%m-%d'))
    
    elif report_type == 'production_daily':
        worksheet = workbook.active
        worksheet.title = "生產日報表"
        
        headers = ["日期", "作業員姓名", "設備名稱", "生產線", "完成數量", "工作時數", "效率 (%)"]
        for col_num, header in enumerate(headers, 1):
            worksheet.cell(row=1, column=col_num, value=header)
        
        for row_num, report in enumerate(data, 2):
            worksheet.cell(row=row_num, column=1, value=report.date.strftime('%Y-%m-%d'))
...
MES 系統團隊
    ...
    # 生成HTML內容
    html_message = render_to_string('reporting/email_report_template.html', {
        'report_type': schedule.get_report_type_display(),
        'report_date': yesterday.strftime('%Y-%m-%d'),
        'data_count': len(data) if not isinstance(data, dict) else sum(len(v) for v in data.values()),
        'data': data,
        'schedule': schedule
    })
...
def send_report_email(email_config, schedule, subject, message, html_message, excel_file):
    """發送報表郵件"""
    try:
        # 設定SMTP連線
