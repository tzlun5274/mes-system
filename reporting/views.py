...
from .models import ProductionDailyReport, OperatorPerformance, ReportSyncSettings, ReportEmailSchedule, ReportEmailLog, ReportingOperationLog
...
reporting_handler.setFormatter(logging.Formatter('%(levelname)s %(asctime)s %(module)s %(message)s'))
...
def production_daily(request):
    ...
    production_reports = ProductionDailyReport.objects.all().order_by('-date', 'operator_name')

    if 'download' in request.GET:
        # 記錄下載操作
        log_user_operation(request.user.username, 'reporting', "下載生產日報表 Excel 文件")
        # 創建 Excel 文件
        ...
        # 寫入表頭
        headers = ["日期", "作業員姓名", "設備名稱", "生產線", "工序名稱", "完成數量", "工作時數", "效率 (件/小時)", "完成率 (%)"]
        ...
        response['Content-Disposition'] = 'attachment; filename=production_daily_report.xlsx'
        workbook.save(response)
        return response

    return render(request, 'reporting/production_daily.html', {
        'production_reports': production_reports,
    })
...
def operator_performance(request):
    ...
    if 'download' in request.GET:
        log_user_operation(request.user.username, 'reporting', "下載作業員生產報表 Excel 文件")
        workbook = openpyxl.Workbook()
        worksheet = workbook.active
        worksheet.title = "作業員生產報表"
        # 新增詳細欄位
        headers = ["作業員名稱", "工單", "產品名稱", "生產數量", "開始時間", "結束時間", "日期", "工序", "設備名稱"]
        for col_num, header in enumerate(headers, 1):
            col_letter = get_column_letter(col_num)
            worksheet[f"{col_letter}1"] = header
        for row_num, report in enumerate(operator_reports, 2):
            ...
            worksheet[f"E{row_num}"] = report.start_time.strftime('%Y-%m-%d %H:%M:%S') if report.start_time else ''
            worksheet[f"F{row_num}"] = report.end_time.strftime('%Y-%m-%d %H:%M:%S') if report.end_time else ''
        ...
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = 'attachment; filename=operator_performance_report.xlsx'
        workbook.save(response)
        return response
    return render(request, 'reporting/operator_performance.html', {
        'operator_reports': operator_reports,
    })
...
def get_production_daily(request):
    # 記錄用戶操作
    log_user_operation(request.user.username, 'reporting', "通過 API 獲取生產日報表數據")
...
def get_operator_performance(request):
    # 回傳所有欄位
    log_user_operation(request.user.username, 'reporting', "通過 API 獲取作業員生產報表數據")
    operator_reports = OperatorPerformance.objects.all()
...
def manual_sync_smt_report(request):
    ...
    return JsonResponse({'success': True, 'message': '已開始同步SMT生產報表，請稍後刷新頁面'})
...
def manual_sync_operator_performance(request):
    ...
    return JsonResponse({'success': True, 'message': '已開始同步作業員生產報表，請稍後刷新頁面'})
    ...
    if request.method == 'POST':
        try:
            ...
            if sync_interval_hours < 1 or sync_interval_hours > 168:  # 1小時到7天
                return JsonResponse({'success': False, 'message': '同步間隔必須在1-168小時之間'})
            ...
            return JsonResponse({'success': False, 'message': '同步間隔必須是有效的數字'})
...
def get_sync_settings(request):
    ...
    try:
        ...
        operator_setting = ReportSyncSettings.objects.filter(report_type='operator').first()
        
        settings = {
            'smt': {
                'sync_interval_hours': smt_setting.sync_interval_hours if smt_setting else 24,
                'is_active': smt_setting.is_active if smt_setting else True
            },
            'operator': {
                'sync_interval_hours': operator_setting.sync_interval_hours if operator_setting else 24,
...
def email_schedule_list(request):
    ...
    schedules = ReportEmailSchedule.objects.all().order_by('report_type', 'schedule_type')
    return render(request, 'reporting/email_schedule_list.html', {
        'schedules': schedules,
    })
...
def email_schedule_create(request):
    ...
    if request.method == 'POST':
        try:
            schedule = ReportEmailSchedule.objects.create(
                ...
                subject_template=request.POST.get('subject_template', 'MES 系統 - {report_type} - {date}'),
                is_active=request.POST.get('is_active') == 'on'
            )
            log_user_operation(request.user.username, 'reporting', f"創建報表郵件發送設定: {schedule}")
    ...
    return render(request, 'reporting/email_schedule_form.html', {
        'title': '創建報表郵件發送設定',
        'action': 'create'
    })
...
def email_schedule_edit(request, schedule_id):
    ...
    if request.method == 'POST':
        try:
            ...
            schedule.subject_template = request.POST.get('subject_template', 'MES 系統 - {report_type} - {date}')
            schedule.is_active = request.POST.get('is_active') == 'on'
            schedule.save()
            
            log_user_operation(request.user.username, 'reporting', f"更新報表郵件發送設定: {schedule}")
...
def email_schedule_delete(request, schedule_id):
    ...
    try:
        ...
        log_user_operation(request.user.username, 'reporting', f"刪除報表郵件發送設定: {schedule_name}")
...
def email_log_list(request):
    ...
    return render(request, 'reporting/email_log_list.html', {
        'logs': logs,
    })
...
def test_send_report_email(request):
    ...
    if request.method == 'POST':
        try:
            ...
            log_user_operation(request.user.username, 'reporting', f"測試發送報表郵件: {schedule}")
            return JsonResponse({'success': True, 'message': f'測試郵件已發送：{schedule}'})
        except ReportEmailSchedule.DoesNotExist:
        ...
        with transaction.atomic():
            ...
            if date_range:
                try:
                    ...
                    messages.error(request, '日期格式錯誤，請使用 YYYY-MM-DD,YYYY-MM-DD 格式')
            ...
            if clear_type == 'all':
                ...
                return redirect('reporting:index')
            # 記錄操作日誌
            _log_operation(f'用戶 {request.user.username} 清除報表資料，類型：{clear_type}，共清除 {cleared_count} 筆記錄')
...
def manufacturing_workhour_list(request):
    ...
    if 'download' in request.GET:
        ...
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = 'attachment; filename=manufacturing_workhour.xlsx'
        with pd.ExcelWriter(response, engine='openpyxl') as writer:
            df.to_excel(writer, index=False)
        return response
    return render(request, 'reporting/manufacturing_workhour_list.html', {
        'workhours': qs,
        'search': search,
    })
