...
class ProductProcessRoute(models.Model):
    ...
    usable_equipment_ids = models.CharField(
        max_length=500, blank=True, verbose_name="指定可用設備 ID 列表"
    )
    dependent_semi_product = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="依賴半成品"
    )
...
class ProductProcessStandardCapacity(models.Model):
    ...
    cycle_time_seconds = models.DecimalField(
        "標準週期時間(秒)", max_digits=6, decimal_places=2
    )
    ...
    learning_curve_factor = models.DecimalField(
        "學習曲線因子", max_digits=3, decimal_places=2
    )

    # 品質因素
    expected_defect_rate = models.DecimalField(
        "預期不良率", max_digits=5, decimal_places=4
    )
    rework_time_factor = models.DecimalField(
        "重工時間因子", max_digits=3, decimal_places=2
    )

    # 版本控制
    version = models.PositiveIntegerField("版本號", default=1)  # 預設為 1，避免為空值
    effective_date = models.DateField(
        "生效日期", auto_now_add=True, null=True, blank=True
    )
    ...
    def get_effective_capacity(self, batch_size=None, actual_efficiency=None):
        ...
        # 應用效率因子
        efficiency = (
            Decimal(str(actual_efficiency))
            if actual_efficiency is not None
            else (self.efficiency_factor or Decimal("1.00"))
        )
        ...
        if batch_size:
            ...
            if batch_size < (self.min_batch_size or 1):
                batch_factor = Decimal(str(batch_size)) / Decimal(
                    str(self.min_batch_size or 1)
                )
                ...
                batch_factor = min(
                    ...
                    Decimal("1")
                    + (
                        Decimal(str(batch_size))
                        - Decimal(str(self.optimal_batch_size or 1))
                    )
                    / Decimal(str(self.optimal_batch_size or 1))
                    * Decimal("0.1"),
                )
    ...
    def get_total_time_for_batch(self, batch_size):
        ...
        if effective_capacity > 0:
            production_time = (Decimal(str(batch_size)) / effective_capacity) * Decimal(
                "60"
            )  # 轉換為分鐘
        ...
        if self.expected_defect_rate and self.expected_defect_rate > 0:
            ...
            rework_time = (
                (defect_quantity / effective_capacity)
                * Decimal("60")
                * (self.rework_time_factor or Decimal("1.00"))
            )
...
class CapacityTemplate(models.Model):
    ...
    cycle_time_seconds = models.DecimalField(
        "標準週期時間(秒)", max_digits=6, decimal_places=2, default=0
    )
    efficiency_factor = models.DecimalField(
        "效率因子", max_digits=3, decimal_places=2, default=1.0
    )

    # 適用範圍
    applicable_products = models.TextField(
        "適用產品類型", blank=True, help_text="產品編號前綴或關鍵字，用逗號分隔"
    )
